...
def get_trial_info(subject, condition, inputs, outputs, recompute, logpath, **kwargs):
    ...
    if save_path.exists() and not recompute:  # only recompute if requested
        ...
        return
    log_path = next(
        Path(".").glob(individualize_path(inputs["log_path"], subject, condition))
    )
    marker_path = next(
        Path(".").glob(individualize_path(inputs["marker_path"], subject, condition))
    )

    sfreq_original = kwargs["sfreq_original"]

    df_log = pd.read_csv(log_path, sep="\t", usecols=["CSI", "shock"])
    markers = read_annotations(marker_path, sfreq_original)
    cues = markers.onset[markers.description == "Stimulus/S  2"]
    assert len(cues) == df_log.shape[0], (
        "Unequal number of trials between"
    ...
    df = pd.DataFrame(
        {"threat": trial_types, "baseline": baselines, "cue": cues, "stimulus": stimuli}
    )
    df = df[
        (df["stimulus"] - df["cue"]) >= 6
    ]  # exclude trials with anticipation windows shorter than 6 seconds
